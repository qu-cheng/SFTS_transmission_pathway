...
#====================================================================================
results.all %>%
  mutate(K.sce = factor(K.sce, levels = c(0.15, 0.05, 0.5)),
         I.sce = case_when(I.sce == 1 ~ "I == 1-5", 
                           I.sce == 2 ~ "I == 5.01-9", 
                           I.sce == 3 ~ "I == 9.01-12"),
         N.sce = case_when(N.sce == 1 ~ "0.01–2",
                           N.sce == 2 ~ "2–5",
                           N.sce == 3 ~ "5–12"),
         K.sce = case_when(K.sce == 0.05 ~ "kappa~'= 0.05'",
                           K.sce == 0.15 ~ "kappa~'= 0.15'",
                           K.sce == 0.5 ~ "kappa~'= 0.5'"),
         N.sce = factor(N.sce, levels = unique(N.sce))) %>%
  arrange(K.sce) %>%
  group_by(I.sce, N.sce, K.sce) %>%
  ...
  facet_grid(rows = vars(K.sce), cols = vars(I.sce),
             switch = "y",
...
ggsave("Figures/Fig4_relative_contribution_barplot.pdf", width = 10, height = 8)
